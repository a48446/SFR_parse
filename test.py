...
class MTFApplication:
    ...
    def display_opencv_stream(self, rtsp_url):
        ...
        print(f"Max resolution: {max_resolution}, Aspect ratio: {aspect_ratio}")
        ...
        self.stream_thread = threading.Thread(target=self.display_opencv_stream_fixed_resolution, args=(rtsp_url,))
        self.stream_thread.daemon = True
        self.stream_thread.start()
    ...
    def export_to_excel(self):
        ...
        for i in range(5):
            data[f'MTF{i + 1}'] = [self.mtf_results[j][i] if len(self.mtf_results[j]) > i else None for j in range(5)]
